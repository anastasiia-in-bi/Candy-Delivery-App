...
class CourierManager(Manager):
    ...
    def create(self, name: str, age: int, courier_type: str) -> list:
        add_sql = f"INSERT INTO courier(name, age, courier_type) " \
                  f"VALUES('{name}', {age}, '{courier_type}') RETURNING *"
    ...
    def update(
            self, courier_id: int, name: str, age: int, courier_type: str
    ) -> list:
        alter_courier = f"UPDATE courier SET name='{name}', age={age}, " \
                        f"courier_type='{courier_type}' " \
                        f"WHERE {courier_id}=courier_id RETURNING *;"

        return self.execute_and_return(alter_courier)

    def delete(self, courier_id: int) -> list:
        del_courier = f"DELETE FROM courier " \
                      f"WHERE courier_id={courier_id} RETURNING *;"
